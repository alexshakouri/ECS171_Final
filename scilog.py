...
def predictor(classifier,test_X,test_Y,realtest_X,idf):
#	indicepo = test_Y>00
#	indicene = test_Y==0
#	test_Y[indicepo]=1
#	test_Y[indicene]=0
	loss1=cross_val_predict(classifier,test_X,test_Y,cv=10)
	loss=classifier.predict(realtest_X)
	indicene = loss<1
	loss[indicene]=0
	realtest_X['loss'] = loss
	realtest_X['id'] = idf
	#test_X = test_X.join(loss)
	realtest_X[['id','loss']].to_csv("pred.csv",index=False)
...
def main():
	...
	#look at predictions
	predictor(clf_model,train_X,train_Y,realtest_X,idf)
	#save prediction
