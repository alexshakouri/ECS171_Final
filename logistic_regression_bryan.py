...
'''
train_x[:,:-1] = np.subtract(train_x[:,522], train_x[:,521])
train_x[:,:-1] = np.divide(np.subtract(train_x[:,272], train_x[:,522]), (np.subtract(train_x[:,522], train_x[:,521])+1))
train_x[:,:-1] = np.divide(train_x[:,269], (np.subtract(train_x[:,522], train_x[:,521])+1))
print train_x.shape
time.sleep(10)
'''
#training
...
classifier = ensemble.GradientBoostingClassifier(n_estimators=n_estimators,random_state=0)
...
classifier.score(sub_test_x, sub_test_y) # accuracy of training validation
